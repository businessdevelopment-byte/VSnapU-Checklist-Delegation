...
const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxGu33pg_zLzyWXoiCgLNq7mIrVxc9FAJSZmyM3Qn-tz3U2A-rm03HYm_SEmNrzpdMP/exec"
...
function AccountDataPage() {
  ...
  const fetchSheetData = async () => {
    try {
      ...
      // Ensure Column Q (col16) exists in headers for Submission Status
      if (!headers.some(h => h.id === 'col16')) {
        headers.push({ id: 'col16', label: 'Submission Status', type: 'string' });
      } else {
        // Optional: Force label if it exists but might be different
        const qHeader = headers.find(h => h.id === 'col16');
        if (qHeader) qHeader.label = 'Submission Status';
      }
      ...
      data.table.rows.forEach((row, rowIndex) => {
        ...
        // Check for specific statuses in Column Q
        const statusVal = columnQValue ? columnQValue.toString().trim() : '';
        const isDone = statusVal === 'DONE';
        const isLeave = statusVal.toLowerCase() === 'leave';

        // Skip rows marked as DONE in column Q (Index 16)
        if (isDone) {
          return;
        }
        ...
        // Explicitly set col16 value if it's "Leave" or if it wasn't captured
        if (isLeave) {
          rowData['col16'] = "Leave";
        } else if (!rowData['col16'] && columnQValue) {
          rowData['col16'] = columnQValue;
        }
        ...
        // For pending tasks: Column L is not null and column M is null, AND not "Leave"
        if (hasColumnL && isColumnMEmpty && !isLeave) {
        ...
        // For history: Both column L and M are not null OR it is "Leave" status
        else if ((hasColumnL && !isColumnMEmpty) || isLeave) {
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      {/* Skip column N (index 13) and show O, P and Q (index 14, 15, 16) */}
                      {sheetHeaders.filter(h => ['col14', 'col15', 'col16'].includes(h.id)).map((header) => (
                        <th
                          key={header.id}
                          className={`px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider
                            ${header.id === 'col14' ? 'bg-blue-50' : ''}
                            ${header.id === 'col15' ? 'bg-purple-50' : ''}
                            ${header.id === 'col16' ? 'bg-red-50' : ''}
                          `}
                        >
                          {header.label}
                        </th>
                    ...
                    {filteredHistoryData.length > 0 ? (
                      filteredHistoryData.map((history) => (
                        <tr key={history._id} className="hover:bg-gray-50">
                          ...
                          {/* Skip column N (index 13) and show O, P and Q */}
                          {sheetHeaders.filter(h => ['col14', 'col15', 'col16'].includes(h.id)).map((header) => (
                            <td
                              key={header.id}
                              className={`px-6 py-4 whitespace-nowrap
                              ...
                              `}
                            >
                              <div className={`text-sm ${history[header.id] === 'Leave' ? 'text-red-600 font-bold' : 'text-gray-900'}`}>
                                {history[header.id] || '—'}
                              </div>
                            </td>
                          ))}
